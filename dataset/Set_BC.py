...
def velocity_profile(
    ...
    if "parabolic" == inlet_type:
        ...
        inlet_field[:, 0] = torch.full_like(inlet_field[:, 0], float(max_u))
        
    elif "uniform_aoa" == inlet_type:
        inlet_field[:, 0:2] = max_u*torch.tensor(
        ...
        inlet_field[:, 0] = max_u*torch.sin(2 * torch.pi * x) * torch.cos(2 * torch.pi * y)
        inlet_field[:, 1] = -max_u*torch.cos(2 * torch.pi * x) * torch.sin(2 * torch.pi * y)
        pressure_field = (-(1/4) * max_u* (torch.cos(4 * torch.pi * x) + torch.cos(4 * torch.pi * y)))[:,None]
